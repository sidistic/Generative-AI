# Generative-AI
Code and Resources that I have used to help you get started out with Generative AI
## Papers
1.  [Attention is all you need](https://arxiv.org/abs/1706.03762)
2.  [Language Models are Few-Shot Learners (OpenAI chatGPT3)](https://arxiv.org/abs/2005.14165) 
3.  [Deep Residual Learning for Image Recognition(uses a method here for better results)](https://arxiv.org/abs/1512.03385)
## Videos
1.  [Andrej Karpathy's GPT from Scratch](https://www.youtube.com/watch?v=kCc8FmEb1nY&t=3506s&ab_channel=AndrejKarpathy)
## Articles
1.  [Residual Blocks (we use something similar in the GPT model)](https://towardsdatascience.com/residual-blocks-building-blocks-of-resnet-fd90ca15d6ec)
